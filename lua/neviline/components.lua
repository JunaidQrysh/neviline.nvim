...
local function separator(symbol, size, condition)
	...
	if condition then
		return string.rep(" ", size, "%#" .. mode_color[mode][2] .. "#" .. symbol .. "%#Comment#")
	end
	return ""
end
...
M.mode = function()
	local mode = vim.api.nvim_get_mode().mode
	local current_mode = "%#" .. mode_color[mode][2] .. "#" .. mode_color[mode][1] .. "%#Comment#"
...
M.interpreter = function(condition)
	...
	if condition then
		return separator("|", 2, condition) .. buf_ft:sub(1, 1):upper() .. buf_ft:sub(2)
	end
	return buf_ft:sub(1, 1):upper() .. buf_ft:sub(2)
end
...
M.LSP_Diagnostics = function()
	...
	local err = (errors and errors > 0) and ("%#St_lspError#" .. icon("", "x-circle", 0, 1) .. errors .. " ") or ""
	local war = (warnings and warnings > 0) and ("%#St_lspWarning#" .. icon("", "alert", 0, 1) .. warnings .. " ")
		or ""
	local hin = (hints and hints > 0) and ("%#St_LspHints#" .. icon("", "light-bulb", 0, 1) .. hints) or ""
	local inf = (info and info > 0) and ("%#St_LspInfo#" .. icon("", "info", 0, 1) .. info .. " ") or ""

	if err ~= "" or war ~= "" or inf ~= "" or hin ~= "" then
		return "%@OpenDiagnosticWorkspace@" .. err .. war .. inf .. hin
	else
		return ""
	end
end
...
M.git = function()
	...
	local added = (git_status.added and git_status.added ~= 0) and (icon("", "diff-added", 1, 1) .. git_status.added)
		or ""
...
M.location = function()
	...
	if c <= 1 or vim.bo.filetype == "neviraideDashboard" or vim.bo.filetype == "TelescopePrompt" then
		return "%#St_Location#" .. icon("Ln", "rows", 1, 2) .. r .. "/" .. lines
	end
	return "%#St_Location#" .. icon("Col", "columns", 0, 2) .. c .. "," .. icon("Ln", "rows", 1, 2) .. r .. "/" .. lines
end
...
M.filesize = function(condition)
	...
	if condition then
		return separator("|", 2, condition) .. "%#St_filesize#" .. string.format(format, size, suffixes[i])
	end
	return ""
end
...
M.fileformat = function(condition)
	...
	if condition then
		return separator("|", 2, condition) .. "%#St_fileformat#" .. symbols[vim.bo.fileformat]
	end
	return ""
end
...
M.encoding = function(condition)
	...
	if condition and buf_enc ~= "" then
		return separator("|", 2, condition) .. "%#St_encoding#" .. string.upper(buf_enc)
	end
	return ""
end
		...
		if count ~= false then
			for _, client in ipairs(vim.lsp.get_clients()) do
				if client.attached_buffers[vim.api.nvim_get_current_buf()] and client.name ~= "null-ls" then
					return separator("|", 2, true) .. "%#Boolean#" .. "%@UpdatePlugins@" .. count
				end
			end
...
M.todos = function()
	...
	if count ~= 0 then
		...
		return " %@ShowTodoList@" .. "%#Boolean#" .. icon("", "tasklist", 0, 2) .. count
	end
	return ""
end
